...
class EmployeeLeaveController extends Controller
{
    ...
    public function index()
    {
        ...
        return inertia("User/PersonelLeave/Index", [
    ...
    public function approve(Request $request, $id)
    {
        ...
        $this->employeeLeaveService->approveLeave($id, $status,  $remarks);

        return redirect()->route('employeeleave.index')->with([
    ...
    public function bulkApprove(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'ids.*' => 'exists:personnel_leaves,id',
        ]);
        $remarks = $request->input('remarks');

        DB::transaction(function () use ($validated, $remarks) {
            foreach ($validated['ids'] as $leaveId) {
                // approve only at current user's level
                $this->employeeLeaveService->approveLeave($leaveId, 'approved',  $remarks);
            }
        });

        return back()->with([
            'success' => 'Selected leave requests approved at your level.',
        ]);
    }
    public function show($id)
    {
        $employeeleave = PersonnelLeave::findOrFail($id);
