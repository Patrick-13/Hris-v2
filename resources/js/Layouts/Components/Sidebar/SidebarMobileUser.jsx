...
const SidebarMobileUser = ({ totalPending, setSidebarOpen }) => {
    ...
    const overttimeItems = [
        {
            submoduleId: 22,
            name: "COC Credit",
            href: route("myovertimecredit.index"),
            active: route().current("myovertimecredit.index"),
        },
        {
            submoduleId: 20,
            name: "Apply RARO",
            ...
            >
                ...
                <nav className="flex-1 mt-6 px-3 space-y-1">
                    {filteredNavItems.map((item) => {
                        const isActive =
                            route().current(item.href) || item.active;
                        return (
                            <Link
                                key={item.name}
                                href={item.href}
                ...
                isActive
                            ...
                            {tkoMenuOpen && (
                                <div className="ml-6 mt-2 space-y-1 border-l border-gray-600 pl-3">
                                    {filterTkoItems.map((sub) => {
                                        ...
                                        const countTko =
                                            sub.submoduleId === 33
                                                ? pendingTkoCount.section +
                                                  pendingTkoCount.division +
                                                  pendingTkoCount.hr
                                                : sub.count || 0;

                                        const showBadgeTko = countTko > 0;
                    ...
                    {hasModule(8) && (
                        <div>
                            ...
                            >
                                <span className="flex items-center gap-3">
                                    <FcLeave size={18} /> Leave Application
                                </span>
                                {employeeMenuOpen ? (
                                    <IoChevronUp size={16} />
                                ) : (
