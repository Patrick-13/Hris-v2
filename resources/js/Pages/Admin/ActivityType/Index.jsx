...
export default function Index({
    ...
    const [selectedActivityType, setSelectedActivityType] = useState(
        activitytypeedits || null
    );

    const handleEditClick = async (activitytypeId) => {
        try {
            const response = await axios.get(
                `/admin/activitytype/${activitytypeId}/edit`
            );
            setSelectedActivityType(response.data); // Set the fetched product data
    ...
    const searchFieldChanged = (field, value) => {
        ...
        router.replace(route("activitytype.index"), {
            method: "get",
    ...
    const sortChanged = (name) => {
        ...
        router.get(route("activitytype.index"), queryParams);
    };

    const deleteActivitytype = (activitytype) => {
        if (
            !window.confirm(
                `are you sure you want to delete the ${activitytype.name} Activity Type?`
            )
        ) {
            return;
        }
        router.delete(route("activitytype.destroy", activitytype.id));
    };
        ...
        >
            ...
            <div className="py-2">
                <div className="max-w-9xl mx-auto sm:px-6 lg:px-8">
                    ...
                    >
                        <Edit
                            activitiestype={selectedActivityType}
