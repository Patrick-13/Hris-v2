...
export default function Index({
    ...
    const handleEditClick = async (companyId) => {
        try {
            const response = await axios.get(`/admin/company/${companyId}/edit`);
            setSelectedCompany(response.data); // Set the fetched product data
    ...
    const sortChanged = (name) => {
        ...
        router.get(route("company.index"), queryParams);
    };

    const deleteCompany = (company) => {
        if (
            !window.confirm(
                `are you sure you want to delete the ${company.name} company?`
            )
        ) {
            return;
        }
