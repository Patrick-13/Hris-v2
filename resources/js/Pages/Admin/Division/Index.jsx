...
export default function Index({
    ...
    const handleEditClick = async (divisionId) => {
        try {
            const response = await axios.get(`/admin/division/${divisionId}/edit`);
            setSelectedDivision(response.data); // Set the fetched product data
    ...
    const searchFieldChanged = (field, value) => {
        ...
        router.replace(route("company.index"), {
            method: "get",
    ...
    const sortChanged = (div_name) => {
        ...
        router.get(route("division.index"), queryParams);
    };

    const deleteDivision = (division) => {
        if (
            !window.confirm(
                `are you sure you want to delete the ${division.div_name} division?`
            )
        ) {
            return;
        }
        router.delete(route("division.destroy", division.id));
    };
                                                    ...
                                                    divisions.data.map(
                                                        (division) => (
                                                            ...
                                                            >
                                                                ...
                                                                <td className="px-3 py-2">
                                                                    {
                                                                        division.employeeBy?.lastname + ", " + division.employeeBy?.firstname
                                                                    }
                                                                </td>
