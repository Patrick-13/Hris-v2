...
export default function Index({
    ...
    const searchFieldChanged = (field, value) => {
        ...
        debounceTimeout.current = setTimeout(() => {
            ...
            router.get(route("iclocktransaction.index"), updatedQueryParams, {
                preserveState: true,
                only: [
                    "transactions",
                    "queryParams",
    ...
    const sortChanged = (employee_name) => {
        ...
        router.get(route("iclocktransaction.index"), queryParams);
    };

    const handleRowsPerPageChange = (e) => {
        const rowsPerPage = parseInt(e.target.value);
        const newParams = { ...queryParams, per_page: rowsPerPage };
        router.get(route("iclocktransaction.index"), newParams);
    };
                                                ...
                                                transactions.data.map(
                                                    (transaction) => (
                                                        ...
                                                        >
                                                            ...
                                                            <td className="px-3 py-2">
                                                                {transaction.employee_transaction ? (
                                                                    `${capitalizeWords(transaction.employee_transaction.last_name || "")}, 
                                                                    ${capitalizeWords(transaction.employee_transaction.first_name || "")}`
                                                                ) : (
                                                                    <span className="text-red-500">
                                                                        NOT IN
                                                                        THE
