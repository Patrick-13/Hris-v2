...
export default function Index({
    ...
    const handleEditClick = async (positionId) => {
        try {
            const response = await axios.get(
                `/admin/position/${positionId}/edit`,
            );
            setSelectedPosition(response.data); // Set the fetched product data
    ...
    const searchFieldChanged = (field, value) => {
        ...
        router.replace(route("position.index"), {
            method: "get",
            data: updatedQueryParams,
        });
    };

    const onKeyPress = (post_name, e) => {
        if (e.key !== "Enter") return;

        searchFieldChanged(post_name, e.target.value);
    };

    const sortChanged = (post_name) => {
        if (post_name === queryParams.sort_field) {
            ...
            queryParams.sort_field = post_name;
            queryParams.sort_direction = "asc";
        }
    ...
    const deletePosition = (position) => {
        if (
            !window.confirm(
                `are you sure you want to delete the ${position.post_name} position?`,
            )
        ) {
            return;
        }
