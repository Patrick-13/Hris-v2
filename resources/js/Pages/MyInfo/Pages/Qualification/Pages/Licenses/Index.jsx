...
export default function Index({
    ...
    const [selectedLicenses, setSelectedLicences] = useState(
        contactdetailedits || null
    );

    const handleEditClick = async (licenseId) => {
        try {
            const response = await axios.get(`/eligibility/${licenseId}/edit`);
            setSelectedLicences(response.data); // Set the fetched product data
    ...
    const deleteWorkEligibility = (licenses) => {
        if (
            !window.confirm(
                `are you sure you want to delete the ${licenses.employee_id} Eligibility/Licenses?`
            )
        ) {
            return;
        }
        router.delete(route("licenses.destroy", licenses.id));
    };
                                ...
                                <div className="flex justify-between items-center gap-4">
                                    <div className="relative flex items-center gap-2">
                                        ...
                                        >
                                            <FaPlus size={14} />
                                            <span>
                                                Add Eligibility/Licenses Details
                                            </span>
                                        </button>
                                    </div>
                                </div>
                                        ...
                                        <tbody>
                                            ...
                                            contactdetails.data.length > 0 ? (
                                                contactdetails.data.map(
                                                    (contactdetail) =>
                                                        contactdetail.employeeEligibilityBy.map(
                                                            (
                                                                eligibility,
                                                                index
                                                            ) => (
                                                                <tr
                                                                    key={`${eligibility.id}-${eligibility.id}-${index}`}
                                                                ...
                                                                >
                                                                    ...
                                                                    <td className="px-3 py-2">
                                                                        {
                                                                            eligibility.placeExamTaken
                                                                        }
                                                                    </td>
                                                                    <td className="px-3 py-2">
                                                                        {
                                                                            eligibility.dateTaken
                                                                        }
                                                                    </td>
