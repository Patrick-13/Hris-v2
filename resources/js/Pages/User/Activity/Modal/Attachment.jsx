...
export default function Attachment({ activityfiles, closeModal }) {
    const { data, setData, post, errors, reset } = useForm({
        activity_id: activityfiles.id,
        activityFile: null,
    });

    const onSubmit = (e) => {
        e.preventDefault();
        post(route("myactivityfile.store"), {
            forceFormData: true,
            onSuccess: () => {
                closeModal();
                reset();
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-1 gap-6">
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setData("activityFile", e.target.files[0])
                                }
                                className="mt-1 block w-full border-gray-300 rounded-md shadow-sm"
                            />
                            <InputError
                                message={errors.activityFile}
                                className="mt-2"
                            />
